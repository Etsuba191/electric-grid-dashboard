...
export function SearchFilters({ assets, filters, onFiltersChange, userRole }: SearchFiltersProps) {
  ...
  const { regions, assetTypes, alertSeverities } = useMemo(() => {
    const typeSet = new Set<string>()
    const sevSet = new Set<string>()

    // Use predefined regions instead of extracting from data
    const regions = [
      "Addis Abeba",
      "Oromiya", 
      "Tigray",
    ...
    // Loop through assets and collect type
              ...
              <div className="space-y-2">
                ...
                <Select value={filters.location} onValueChange={(value) => updateFilter("location", value)}>
                  ...
                  <SelectContent className="bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 max-h-64 overflow-auto">
                    {regions.length === 0 ? (
                      <div className="px-3 py-2 text-sm text-blue-600 dark:text-slate-500">No regions</div>
                    ) : (
                      regions.map((region) => (
                      ...
                      assetTypes.map((type) => (
                        <SelectItem key={type} value={type}>
                          <div className="flex items-center space-x-2">
                            ...
                            <span>
                              {type === "tower" ? "Electric Tower" : 
                               type === "substation" ? "Substation" :
                               type === "generation_plant" ? "Generation Plant" :
                               type === "transmission_line" ? "Transmission Line" : type}
                            </span>
                          </div>
                        </SelectItem>
                      ))
