...
export function Analytics({ assets }: AnalyticsProps) {
  const { statusData, typeData, voltageHistogram, sourceCounts } = useMemo(() => {
    ...
    // Type data: simply counts towers vs substations
    const typeData = [
    ...
    const sourceCounts = [
      ...
      { name: "Towers", value: towers.length, color: "#34D399" },
    ]

    return { statusData, typeData, voltageHistogram, sourceCounts }
  }, [assets])
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="overview" className="w-full">
        <TabsList className="grid w-full grid-cols-4 bg-slate-200 dark:bg-slate-700">
